...
@section('title')
{{ trans('tattendance_lang.panel_title') }}
@endsection
@section('content')
<!--main content start-->
<body onload="lll()">
  <div class="content-wrapper">
    ...
    <section class="content">
            <div class="row">
                  <div class="col-lg-12">
                      ...
                      <ul class="breadcrumb">
                          ...
                          <li class="active">{{ trans('tattendance_lang.panel_title') }}</li>
                      </ul>
                      <!--breadcrumbs end -->
                  </div>
              </div>
        ...
        <tbody>
            <form method="post" action="{{ url('select_teacher_atendance') }}" class="form">
    ...
    <div class="col-sm-offset-3 col-md-6">
        <table class="table table-bordered">
            ...
            <tbody>
                        ...
                        <tr class="gradeA">
                            <td data-title="">{{ $teach->name }}</td>
                            
                            <td data-title="">{{ $teach->email }}</td>
                            <td data-title="">
                            <?php 
                                //$at = App\Attendance::where('teacher_id',$teach->id)->where('date',$full_date)->find($teach->id);
                            $at = App\TeacherAttendance::orderBy('created_at','desc')->paginate()->where('teacher_id',$teach->id)->where('date',$full_date);
...
<?php
 $get = App\TeacherAttendance::orderBy('created_at','desc')->paginate(5)->where('teacher_id',$teach->id)->where('date',$full_date);
 if($get->count() > 0){ ?>
 <form method="post" 
    action="{{ url('teacher_attendance/update') }}/<?php echo $date.'/'.$month.'/'.$year;?>">
       <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <div class="col-sm-offset-3 col-md-6">
            <table  class="table table-bordered">
                <thead>
                ...
                <tbody>
                    ...
                    @foreach ($teacher as $teach)
                        <tr class="gradeA">
                            ...
                            <td align="center">
                                <?php 
                                $at = App\TeacherAttendance::where('teacher_id', $teach->id)->where('date', $full_date)->find($teach->id);
                                if($at){
                                    $status = $at->status; 
                                }
                                    ?>
...
<form method="post" 
    action="{{ url('teacher_attendance') }}/<?php echo $date.'/'.$month.'/'.$year;?>">
       <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <div class="col-sm-offset-3 col-md-6">
            <table  class="table table-bordered">
                <thead>
                        ...
                        <tr class="gradeA">
                            ...
                            <td align="center">
                                ...
                                if($at){
                                    ...
                                    ?>
                                <input type="hidden" name="stu[]" value="{{ $teach->id }}">
                                    <select name="status[]" class="form-control" style="width:100px; float:left;">
                                        <option value="0"></option>
                                        <option value="1">Present</option>
                                        <option value="0">Absent</option>
