                    ...
                    <tr>
                        <td valign="top" class="md-left no-padding no-border" style="padding-right:10px">
                                ...
                                <tbody>
                                    ...
                                    @foreach($subjects as $key => $subject)
                                        ...
                                        <tr>
                                            <td>{{$key+1}}</td>
                                            <td>{{$subject->title}}</td>
                                            <td>{{$the_first_test ? $the_first_test->first_a+$the_first_test->first_b+$the_first_test->first_c: '-'}}</td>
                                            <td>{{$the_first_test ? $the_first_test->second: '-'}}</td>
                                            <td>{{$student->mark->where('subject_id', $subject->id)->first() ? $student->mark->where('subject_id', $subject->id)->first()->mark_obtained : '-'}}</td>
                                            <td>{{$total}}%</td>
                                            <td>{{App\Mark::checkgradeSpreadsheet($total)}}</td>
                                            <td>{{App\Mark::checkgradeSpreadsheetName($total)}}</td>
                            ...
                            <table style="margin-top:10px">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>MAXIMUM OBTAINABLE:{{$subjects ? $subjects->count()*100 : ''}}</td>
                                        <td>AVERAGE SCORE: {{$subjects ? ($cummulative/($subjects->count()*100))*100 : ''}}%</td>
                                        
                                        <td>({{App\Mark::checkgradeSpreadsheet(($cummulative/($subjects->count()*100))*100)}}) {{App\Mark::checkgradeSpreadsheetName(($cummulative/($subjects->count()*100))*100)}}</td>
                                    </tr>
                                </tbody>
                            </table>
                            <div style="padding:10px">
                               <strong>VACATION DATE: {{$student->mark->first() ? $student->mark->first()->vacation_date : '-'}}
                                <span style="margin-left:40px">  NEXT RESUMPTION DATE: {{$student->mark->first() ? $student->mark->first()->resumption : '-'}}</span></strong> 
                            </div>
                            ...
                            <table style="margin-top:10px;">
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="4">
                                            <h3>{{$student->mark->first() ? strtoupper($student->mark->first()->teacher_comment) : '-'}}</h3> 
                                            <p>By {{$student->mark->first() ? App\User::find($student->mark->first()->teacher_id) ? App\User::find($student->mark->first()->teacher_id)->name : App\User::find($class->teacher_id) ? App\User::find($class->teacher_id)->name: '-' : '-'}} on {{$student->mark->first() ? $student->mark->first()->created_at->format('F d, Y') : '-'}}</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table style="margin-top:10px;">
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="4">
                                            <h3>{{$student->mark->first() ? strtoupper($student->mark->first()->principal_comment) : '-'}}</h3> 
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <td valign="top" class="md-right no-padding no-border">
                            ...
                            <div>
                                ...
                                <p>{{$student->mark->first() ? $student->mark->first()->created_at->format('F d, Y') : '-'}}</p>
                            </div>
                        </td>
                    </tr>
                </tbody>
